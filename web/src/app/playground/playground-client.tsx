...
import { Send, AlertTriangle, CheckCircle, Settings } from 'lucide-react'
...
let cachedEvaluators: Evaluator[] | null = null
let cacheTimestamp: number = 0
const CACHE_DURATION = 30000 // 30 seconds

export default function PlaygroundClient() {
  const [prompt, setPrompt] = useState('')
  const [evaluators, setEvaluators] = useState<Evaluator[]>(cachedEvaluators || [])
  const [selectedEvaluatorIds, setSelectedEvaluatorIds] = useState<string[]>([])
  const [isLoadingEvaluators, setIsLoadingEvaluators] = useState(!cachedEvaluators)
  ...
  useEffect(() => {
    const loadEvaluators = async () => {
      ...
      const now = Date.now()
      if (cachedEvaluators && (now - cacheTimestamp) < CACHE_DURATION) {
        setEvaluators(cachedEvaluators)
        setIsLoadingEvaluators(false)

        // Select default evaluators
        const defaultEvaluators = cachedEvaluators
      ...
      try {
        ...
        // Update cache
        cachedEvaluators = data
        cacheTimestamp = Date.now()

        setEvaluators(data)

        // Select a few default evaluators
        const defaultEvaluators = data
          .filter((e: Evaluator) => ['rule_based', 'pii_regex'].includes(e.kind))
          .slice(0, 2)
          .map((e: Evaluator) => e.id)
        setSelectedEvaluatorIds(defaultEvaluators)
    ...
    <div className="max-w-7xl mx-auto px-6 space-y-6">
      ...
      {selectedEvaluatorIds.length === 0 && evaluators.length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 flex items-start space-x-3">
          ...
          <div>
            <h3 className="text-orange-800 font-medium">No Evaluators Selected</h3>
